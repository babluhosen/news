...
$smedit=$_GET['id'];
//select query//
$selectquery="SELECT * FROM tble_news WHERE c_id=$smedit";
$data=$conn->query($selectquery);
if($data->num_rows>0){
 
 while($final=$data->fetch_assoc()){    //while looping kore arry fetch kore repaing hmtl code breaket close
...
<?php
  ...
  header("location:index.php");//return to category
}
